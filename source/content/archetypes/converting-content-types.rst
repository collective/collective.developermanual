...
In this example we take a folder of "Pages" (meta type - Document) and create 
News Items from them.

::

    from the News Item type, the new items
    will be content copies of their coresponding Pages (Documents)"""
    ...
    pages = context.listFolderContents(contentFilter={"portal_type" : source_contenttype})
...
::
    ...
    for item in items:
        ...
        service = context.invokeFactory(target_contenttype, id, title=title,
                           description=description,imageCaption=imageCaption,
